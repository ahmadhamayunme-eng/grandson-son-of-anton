...
<style>
  ...
  .card-h h3{margin:0;font-size:1.85rem}
  ...
  .list li{display:grid;grid-template-columns:1fr auto auto;gap:10px;align-items:center;padding:11px 14px;border-top:1px solid rgba(255,255,255,.07)}
  .list .sub{font-size:.9rem;color:rgba(255,255,255,.6)}
  .avatars{display:inline-flex;align-items:center}
  .avatar{width:29px;height:29px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#d8d2c2,#ad9f85);color:#2b261b;font-size:.68rem;font-weight:700;border:1px solid rgba(255,255,255,.18);margin-left:-6px}
  ...
  .is-blocked{background:rgba(161,131,47,.24);color:#f0d071}
  .is-progress{background:rgba(101,78,157,.26);color:#c9b4ff}
  .is-done{background:rgba(62,141,98,.24);color:#82d79f}
  .is-todo{background:rgba(68,97,137,.24);color:#8eb7f2}

  .phase-wrap{display:grid;gap:12px}
  .phase{border:1px solid rgba(255,255,255,.08);border-radius:12px;background:linear-gradient(160deg,rgba(26,27,38,.7),rgba(18,19,27,.62));overflow:hidden}
  .phase h4{margin:0;font-size:1.6rem}
  .pv-table{width:100%;border-collapse:collapse}
  .pv-table th,.pv-table td{padding:9px 12px;border-top:1px solid rgba(255,255,255,.07)}
  .pv-table th{color:rgba(255,255,255,.65);font-size:.82rem;text-transform:uppercase}

  .docs-toolbar{display:grid;grid-template-columns:1fr auto auto;gap:10px;margin-bottom:12px}
  .docs-search{padding:10px 12px;border:1px solid rgba(255,255,255,.1);border-radius:10px;background:rgba(255,255,255,.03);color:#eee}
  .docs-section{display:grid;gap:10px}
  .doc-item{display:grid;grid-template-columns:1fr auto auto;gap:10px;align-items:center;padding:10px 14px;border-top:1px solid rgba(255,255,255,.07)}

  .activity .row-item{display:grid;grid-template-columns:36px 1fr auto;gap:10px;align-items:flex-start;padding:11px 14px;border-top:1px solid rgba(255,255,255,.07)}
  .ico{width:34px;height:34px;border-radius:50%;display:grid;place-items:center;background:rgba(255,255,255,.1)}
...
<section class="pv-shell">
  <header class="pv-top">
    <div>
      ...
      <div class="pv-sub">
        <span><?=h($project['client_name'])?></span>
        <span>•</span>
  ...
  <?php if($tab==='overview'): ?>
    <div class="pv-grid">
      <section class="glass">
        <div class="card-h"><h3>Overview</h3></div>
        <div class="ov-body">
          <div class="text-muted">Progress</div>
          <div class="prog"><span style="width: <?=$progress?>%"></span></div>
          <div><?=$progress?>% complete (<?=$doneTasks?>/<?=$totalTasks?> tasks)</div>
          <div class="mt-3 text-muted">Summary</div>
          <p class="mb-0"><?=h($project['notes'] ?: 'Project overview and delivery milestones are tracked through tasks and docs for this workspace.')?></p>
          <div class="meta">
            <div><div class="text-muted small">Last Updated</div><div><?=h(format_date($project['updated_at']))?></div></div>
            <div><div class="text-muted small">Created</div><div><?=h(format_date($project['created_at']))?></div></div>
          </div>
        </div>
      </section>
      <div class="d-grid gap-3">
        <section class="glass">
          <div class="card-h"><h3>Active Tasks</h3><a class="btn btn-sm btn-outline-light" href="project_view.php?id=<?=$id?>&tab=tasks">View</a></div>
          <ul class="list">
            <?php foreach(array_slice($tasks,0,5) as $t): ?>
              <li>
                <div><div><?=h($t['title'])?></div><div class="sub"><?=h($t['assignee_names'] ?: 'Unassigned')?></div></div>
                <span class="pill <?=pv_status_class((string)$t['status'])?>"><?=h($t['status'])?></span>
                <span><?=h($t['due_date']?format_date($t['due_date']):'—')?></span>
              </li>
            <?php endforeach; ?>
            <?php if(!$tasks): ?><li><div class="text-muted">No tasks yet.</div></li><?php endif; ?>
          </ul>
        </section>

        <section class="glass">
          <div class="card-h"><h3>Docs</h3><a class="btn btn-sm btn-outline-light" href="project_view.php?id=<?=$id?>&tab=docs">Open</a></div>
          <ul class="list">
            <?php foreach(array_slice($docs,0,5) as $d): ?>
              <li>
                <div><div><?=h($d['title'])?></div><div class="sub">By <?=h($d['author_name'] ?: 'Unknown')?></div></div>
                <span class="sub"><?=h(format_date($d['updated_at']))?></span>
                <a class="btn btn-sm btn-outline-light" href="doc_edit.php?id=<?=$d['id']?>">Open</a>
              </li>
            <?php endforeach; ?>
            <?php if(!$docs): ?><li><div class="text-muted">No documents yet.</div></li><?php endif; ?>
          </ul>
        </section>
      </div>
    </div>
  ...
  <?php elseif($tab==='tasks'): ?>
    <section class="phase-wrap">
      <?php foreach($phases as $ph): ?>
        <article class="phase">
          <div class="card-h"><h4><?=h($ph['name'])?></h4><?php if($can_manage): ?><button class="btn btn-sm btn-yellow" data-bs-toggle="modal" data-bs-target="#addTask">+ Add Task</button><?php endif; ?></div>
          <table class="pv-table">
            <thead><tr><th>Task</th><th>Status</th><th>Assignees</th><th>Due</th><th>Actions</th></tr></thead>
            <tbody>
              <?php foreach(($by_phase[$ph['id']] ?? []) as $t): ?>
                <tr>
                  <td><?=h($t['title'])?></td>
                  <td><span class="pill <?=pv_status_class((string)$t['status'])?>"><?=h($t['status'])?></span></td>
                  <td><?=h($t['assignee_names'] ?: '—')?></td>
                  <td><?=h($t['due_date'] ? format_date($t['due_date']) : '—')?></td>
                  <td><a class="btn btn-sm btn-outline-light" href="task_view.php?id=<?=$t['id']?>">Open</a></td>
                </tr>
              <?php endforeach; ?>
  ...
  <?php elseif($tab==='docs'): ?>
    <section>
      <form class="docs-toolbar" method="get">
        <input type="hidden" name="id" value="<?=$id?>"><input type="hidden" name="tab" value="docs">
        <input class="docs-search" name="dq" value="<?=h($docsQ)?>" placeholder="Search documents...">
        <?php if($can_manage): ?><button type="button" class="btn btn-yellow" data-bs-toggle="modal" data-bs-target="#addDoc">+ New Doc</button><?php endif; ?>
        <a class="btn btn-outline-light" href="docs.php?project_id=<?=$id?>">Upload</a>
      </form>
      <div class="glass docs-section">
        <div class="card-h"><h3>Project Documents</h3><span><?=count($docs)?> files</span></div>
        <?php foreach($docs as $d): ?>
          <div class="doc-item">
            <div>
              <div><?=h($d['title'])?></div>
              <div class="sub">Updated <?=h(format_date($d['updated_at']))?> by <?=h($d['author_name'] ?: 'Unknown')?></div>
            </div>
            <span class="text-muted"><?=strlen((string)($d['content'] ?? ''))?> chars</span>
            <a class="btn btn-sm btn-outline-light" href="doc_edit.php?id=<?=$d['id']?>">Open</a>
          </div>
        <?php endforeach; ?>
        <?php if(!$docs): ?><div class="p-4 text-muted">No documents found.</div><?php endif; ?>
      </div>
    </section>

  <?php else: ?>
    <section class="glass activity">
      <div class="card-h"><h3>Activity</h3><span><?=count($activities)?> events</span></div>
      <?php foreach($activities as $a): ?>
        <div class="row-item">
          <div class="ico"><?=h(strtoupper(substr((string)$a['source'],0,1)))?></div>
          <div>
            <div><?=h($a['message'])?></div>
            <div class="sub">by <?=h($a['actor'])?></div>
          </div>
          <div class="text-muted"><?=h(format_date($a['happened_at']))?></div>
        </div>
      <?php endforeach; ?>
      <?php if(!$activities): ?><div class="p-4 text-muted">No activity yet.</div><?php endif; ?>
    </section>
  <?php endif; ?>
</section>
...
<div class="modal fade" id="addPhase" tabindex="-1">
  <div class="modal-dialog"><div class="modal-content card p-3">
    ...
    <form method="post"><input type="hidden" name="csrf" value="<?=h(csrf_token())?>"><input type="hidden" name="add_phase" value="1">
      <div class="modal-body"><label class="form-label">Phase Name</label><input class="form-control" name="phase_name" required></div>
      <div class="modal-footer border-0"><button class="btn btn-outline-light" type="button" data-bs-dismiss="modal">Cancel</button><button class="btn btn-yellow" type="submit">Add</button></div>
    </form>
  </div></div>
</div>

<div class="modal fade" id="addTask" tabindex="-1">
  <div class="modal-dialog modal-lg"><div class="modal-content card p-3">
    ...
    <form method="post"><input type="hidden" name="csrf" value="<?=h(csrf_token())?>"><input type="hidden" name="add_task" value="1">
      <div class="modal-body"><div class="row g-3">
        <div class="col-md-8"><label class="form-label">Title</label><input class="form-control" name="title" required></div>
        <div class="col-md-4"><label class="form-label">Phase</label><select class="form-select" name="phase_id" required><?php foreach($phases as $ph): ?><option value="<?=$ph['id']?>"><?=h($ph['name'])?></option><?php endforeach; ?></select></div>
        ...
        <div class="col-md-4"><label class="form-label">Status</label><select class="form-select" name="status"><?php foreach($statuses as $s): ?><option value="<?=h($s)?>"><?=h($s)?></option><?php endforeach; ?></select></div>
        <div class="col-md-4"><label class="form-label">Due Date</label><input class="form-control" type="date" name="due_date"></div>
        <div class="col-md-4"><label class="form-label">Assignees</label><select class="form-select" name="assignees[]" multiple><?php foreach($team as $m): ?><option value="<?=$m['id']?>"><?=h($m['name'])?> (<?=h($m['role_name'])?>)</option><?php endforeach; ?></select></div>
      </div></div>
      <div class="modal-footer border-0"><button class="btn btn-outline-light" type="button" data-bs-dismiss="modal">Cancel</button><button class="btn btn-yellow" type="submit">Create Task</button></div>
    </form>
  </div></div>
</div>
...
<div class="modal fade" id="addDoc" tabindex="-1">
  <div class="modal-dialog modal-lg"><div class="modal-content card p-3">
    <div class="modal-header border-0"><h5 class="modal-title">New Doc</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
    <form method="post"><input type="hidden" name="csrf" value="<?=h(csrf_token())?>"><input type="hidden" name="create_doc" value="1">
      <div class="modal-body"><div class="mb-3"><label class="form-label">Title</label><input class="form-control" name="doc_title" required></div><div class="mb-3"><label class="form-label">Content</label><textarea class="form-control" name="doc_content" rows="10"></textarea></div></div>
      <div class="modal-footer border-0"><button class="btn btn-outline-light" type="button" data-bs-dismiss="modal">Cancel</button><button class="btn btn-yellow" type="submit">Create Doc</button></div>
    </form>
  </div></div>
</div>
<?php endif; ?>
