...
<style>
  ...
  .panel h3 { font-size: 14px; font-weight: 600; color: var(--text); margin-bottom: 14px; display: flex; align-items: center; justify-content: space-between; gap: 12px; }
...
<div class="page-header">
  <div class="page-header-left">
    ...
    <div>
      ...
      <div class="page-sub">Define project workflow statuses used across boards and reports.</div>
    </div>
  </div>
</div>
...
<div class="body-wrap">
  <div class="panel">
    ...
    <form method="post">
      ...
      <div class="form-row"><input class="input" name="name" placeholder="e.g. In Progress" required></div>
      <button class="btn btn-primary btn-block save-flash" type="submit">Add Project Status</button>
    </form>
  </div>
  ...
  <div class="panel" style="padding:0;overflow:hidden;">
    <div style="padding:14px 18px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--border);">
      <div style="font-size:14px;font-weight:600;color:var(--text);">Current Statuses (<?= count($rows) ?>)</div>
      <div class="panel-actions">
        <button type="submit" form="reorderForm" class="btn-tiny">Save Order</button>
        <button type="submit" form="bulkDelForm" class="btn-tiny danger" onclick="return confirm('Delete selected statuses permanently?');">Delete Selected</button>
      </div>
    </div>
    ...
    <table class="gen">
      <thead><tr><th style="width:48px;">Move</th><th style="width:44px;text-align:center;"><input type="checkbox" onclick="document.querySelectorAll('.status-check').forEach(cb => cb.checked = this.checked)"></th><th>Name</th><th>Sort</th><th style="text-align:right;width:140px;">Actions</th></tr></thead>
      <tbody id="sortableRows">
      <?php if (!$rows): ?>
        <tr><td colspan="5" class="empty-state">No statuses yet.</td></tr>
      <?php else: foreach ($rows as $r): ?>
        <tr draggable="true" data-id="<?= (int)$r['id'] ?>">
          <td><span class="drag-handle">⠿</span></td>
          <td style="text-align:center;"><input class="status-check" type="checkbox" name="ids[]" value="<?= (int)$r['id'] ?>" form="bulkDelForm"></td>
          <td>
