...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'bulk_delete') {
    ...
    if (!$ids) { flash_set('error', 'Select at least one status to delete.'); redirect('settings_project_statuses.php'); }
    $marks = implode(',', array_fill(0, count($ids), '?'));
    try {
      $st = $pdo->prepare("DELETE FROM project_statuses WHERE workspace_id = ? AND id IN ($marks)");
      $st->execute(array_merge([$ws], $ids));
      flash_set('success', $st->rowCount() . ' status(es) deleted permanently.');
  ...
  if ($action === 'update') {
    ...
    if ($id <= 0 || $name === '') { flash_set('error', 'Valid status name required.'); redirect('settings_project_statuses.php'); }
    $pdo->prepare('UPDATE project_statuses SET name=?, updated_at=? WHERE workspace_id=? AND id=?')
  ...
  if ($action === 'reorder') {
    ...
    if (!$order) { flash_set('error', 'No statuses selected for sorting.'); redirect('settings_project_statuses.php'); }
  ...
  flash_set('success', 'Project status added.');
  redirect('settings_project_statuses.php');
}

$rows = $pdo->query("SELECT * FROM project_statuses WHERE workspace_id=$ws ORDER BY sort_order ASC")->fetchAll();

// Usage counts per status
$usage = [];
try {
  $us = $pdo->prepare('SELECT status_id, COUNT(*) AS c FROM projects WHERE workspace_id=? GROUP BY status_id');
  $us->execute([$ws]);
  foreach ($us->fetchAll() as $u) { $usage[(int)$u['status_id']] = (int)$u['c']; }
} catch (Throwable $e) { $usage = []; }

/** Pick a status color based on the status name (so the visual swatch is meaningful). */
function sps_status_color(string $name): array {
  $s = strtolower($name);
  if (strpos($s, 'active') !== false || strpos($s, 'progress') !== false || strpos($s, 'open') !== false) return ['#22c55e', 'Active'];
  if (strpos($s, 'hold') !== false || strpos($s, 'pause') !== false) return ['#f59e0b', 'Paused'];
  if (strpos($s, 'complete') !== false || strpos($s, 'done') !== false || strpos($s, 'closed') !== false) return ['#3b82f6', 'Done'];
  if (strpos($s, 'cancel') !== false || strpos($s, 'archive') !== false || strpos($s, 'dead') !== false) return ['#6b7280', 'Archived'];
  if (strpos($s, 'review') !== false || strpos($s, 'pending') !== false) return ['#a855f7', 'Review'];
...
<div class="settings-topbar">
  ...
  <span class="sep">/</span>
  <span class="current">Project Statuses</span>
</div>

<div class="page-header">
  <div class="page-header-left">
    <div>
      <div class="page-title">Project Statuses</div>
      <div class="page-sub">Workflow status values used across projects, boards, and reports.</div>
    </div>
  </div>
...
<div class="settings-tabs">
  ...
  <a class="tab active" href="settings_project_statuses.php"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="3"></circle></svg>Project Statuses</a>
...
<div class="body-stack">
  ...
  <div class="panel">
    ...
    <form method="post" class="add-form">
      ...
      <span class="icon-picker-btn" title="Project status">
        <svg class="glyph" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="3"></circle></svg>
      </span>
  ...
  <div class="panel">
    <div class="panel-head">
      ...
      <div class="panel-actions">
        <button type="submit" form="bulkDelForm" class="btn btn-danger" onclick="return confirm('Delete selected statuses permanently?');">
    ...
    <table class="list-table">
      ...
      <tbody id="rows">
        <?php if (!$rows): ?>
          <tr class="empty-row"><td colspan="7">No project statuses yet.</td></tr>
        <?php else: foreach ($rows as $r):
          $cnt = $usage[(int)$r['id']] ?? 0;
          [$color, $kind] = sps_status_color((string)$r['name']);
        ?>
          <tr data-id="<?= (int)$r['id'] ?>">
    ...
    <div class="order-hint">
      ...
      <span>Drag the <b>handle</b> to reorder. Color swatches are auto-derived from the status name. Deleting a status in use will leave existing projects untagged — they won't be deleted.</span>
    </div>
  </div>
</div>
