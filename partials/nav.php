...
<style>
  ...
  .brand-badge {
    ...
    display: grid;
    place-items: center;
    border: 1px solid rgba(246,212,105,.55);
    color: #f6d469;
    font-size: 1.25rem;
    line-height: 1;
  }
  ...
  .sidebar-label {
    ...
    font-size: .82rem;
    color: rgba(236,236,240,.62);
    letter-spacing: .08em;
    font-weight: 600;
    padding: 0 10px;
  }
  .sidebar-dot-item {
    color: rgba(236,236,240,.72);
    display: block;
  ...
  .sidebar-dot-item::before {
    content: '';
    width: 8px;
    height: 8px;
    border-radius: 50%;
    background: rgba(240,240,244,.85);
    position: absolute;
    left: 12px;
    top: 50%;
  ...
  .sidebar-avatar {
    ...
    height: 36px;
    border-radius: 999px;
    background: linear-gradient(135deg, #5b6c9b, #374263);
    color: #fff;
    font-weight: 600;
    display: grid;
    place-items: center;
  ...
  <div class="sidebar-wrap">
    ...
    <nav class="nav flex-column">
      ...
      <?php if ($role === 'CTO' || $role === 'Super Admin'): ?>
        ...
        <a class="sidebar-dot-item" href="finance.php">Finance Dashboard</a>
        <a class="sidebar-dot-item" href="cto_submit.php">Submit to Client</a>
      <?php endif; ?>

      <?php if (auth_can_finance()): ?>
        <div class="sidebar-label">Finance</div>
        <a class="sidebar-dot-item" href="payments_received.php">Payments Received</a>
        <a class="sidebar-dot-item" href="unreceived_payments.php">Unreceived Payments</a>
        <a class="sidebar-dot-item" href="project_expenses.php">Project Expenses</a>
      <?php endif; ?>

      <?php if (in_array($role, ['CEO','CTO','Super Admin'], true)): ?>
        <div class="sidebar-label">Admin</div>
        <a class="sidebar-dot-item" href="users_management.php">Users</a>
        <a class="sidebar-dot-item" href="settings.php">Settings</a>
      <?php endif; ?>
